...
def save_upload(file):
    ...
    # Якщо папка завантажень стандартна, віддаємо через /static/uploads/...
    # Якщо пізніше перенесемо фото у Cloudinary/Supabase Storage, цю функцію замінимо.
...
def reveal(game_id):
    ...
    if q and option_id:
        db().execute(
            'INSERT OR IGNORE INTO revealed(game_id,question_id,option_id) VALUES(?,?,?)',
...
def invite(code):
    ...
    if request.method == 'POST':
        ...
        cur = db().execute(
            'INSERT INTO players(game_id,name,avatar,created_at) VALUES(?,?,?,?)',
            (game['id'], name, avatar, datetime.now().strftime('%H:%M'))
        )
        db().commit()
        session[f'player_{game["id"]}'] = cur.lastrowid
